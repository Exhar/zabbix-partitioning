...
echo -e "\n"
        echo "GRANT LOCK TABLES ON zabbix.* TO '${DBUSER}'@'${DBHOST}' IDENTIFIED BY '${DBPASS}';" | mysql -h${DBHOST} -u${DBUSER} --password=${DBPASS}
...
_EOF_
        ...
        if [ "$where" = "Y" ]; then
                cat >/etc/cron.d/zabbix-housekeeping <<_EOF_
59 23 * * * root /bin/bash /etc/zabbix/cron.d/housekeeping.sh
_EOF_
                chmod +x /etc/cron.d/zabbix-housekeeping
        else
                crontab -l >$tmpfile
                cat >>$tmpfile <<_EOF_
0 0 * * *  /etc/zabbix/cron.d/housekeeping.sh
